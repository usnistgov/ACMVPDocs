...
0. Login to the system via `POST /amvp/v1/login`
...
Also note the existence of the `GET /amvp/v1/schemas/&lt;endpoint&gt;/&lt;optionally, version&gt;` endpoint, which provides a list of schema versions for each endpoint and additionally provides the associated schema itself when the version is appended to the request. For example, a user could run `GET /amvp/v1/schemas/securityPolicy` to see versions of the Security Policy submission schema, and `GET /amvp/v1/schemas/securityPolicy/0.2` to see specifically schema version 0.2 for the Security Policy submission. Currently, `certificateRequestRegister`, `evidence`, `sourceCode`, `otherDocumentation`, and `securityPolicy` are all supported.
